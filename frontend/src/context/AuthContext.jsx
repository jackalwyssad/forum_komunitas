...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const handleIdleLogout = () => {
      logout();
      alert(
        "Sesi Anda telah berakhir karena tidak ada aktivitas (idle). Silakan login kembali.",
      );
  ...
  const sendOtp = async (name, email, password, password_confirmation) => {
    return await api.post("/register/send-otp", {
      name,
      email,
      password,
  ...
  return (
    <AuthContext.Provider
      value={{ user, loading, login, sendOtp, verifyOtp, logout, isAdmin }}
    >
      {children}
    </AuthContext.Provider>
  );
